        ...
        <div className="relative w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl bg-white dark:bg-gray-800 rounded-xl text-left overflow-hidden shadow-xl transform transition-all max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-3 sm:p-4 md:p-5">
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              {isDeployed && (
                <ModalActionButton href={project.liveUrl} icon={<ExternalLink size={16} />} gradient>
                  {t('common.viewProject')}
                </ModalActionButton>
              )}
              <ModalActionButton href={project.githubUrl} icon={<Github size={16} />}>
                {t('common.viewCode')}
              </ModalActionButton>
            </div>
          </div>
